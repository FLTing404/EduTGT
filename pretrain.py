...
edges, num_nodes, nodes_list, node_time,train_data,_,_,_,_ = Dataset(file=ml_path)
...
if _rel:
    print(f"[pretrain] pres_relation: {_rel} (pres_mix_uniform={args.pres_mix_uniform})")
...
alpha = 0.6
MODEL_SAVE_PATH = str(paths.pretrain_ckpt_path(data_name))
MIDDLE_PATH = str(paths.middle_ckpt_path(data_name))
...
best_model_path = str(paths.checkpoint_ckpt_path(data_name))
