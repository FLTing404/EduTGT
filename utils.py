...
def get_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help='含 ml_*.csv 与 *.content 的数据目录（相对项目根或绝对路径），例如 data/processed/data_AAA_2013J',
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help='课程实例关系 JSON（W 矩阵）；默认若 data_dir 下存在 pres_relation.json 则自动使用',
    )
    parser.add_argument(
        '--pres_mix_uniform',
        type=float,
        default=0.35,
        help='按 W 偏置负样本 dst 时混入均匀分布的比例（0 全按 W，1 等价均匀）',
    )
...
class EarlyStopping:
    def __init__(self, dn, max_round=3, higher_better=True, tolerance=1e-10):
        ...
        self.path = str(paths.checkpoint_ckpt_path(dn))
...
def resolve_training_data(args):
    """
    返回 (ml_csv_path, content_path, data_name)。
    data_name 用于 checkpoint / 预训练权重文件名。
    """
    root = project_root()
    if getattr(args, "data_dir", None):
        ...
        ml_path = ml_files[0]
        if len(ml_files) > 1:
            print(f"警告: 多个 ml_*.csv，使用 {os.path.basename(ml_path)}")
        ...
        if len(content_files) > 1 and not preferred:
            print(f"警告: 多个 *.content，使用 {os.path.basename(content_path)}")
        data_name = os.path.basename(os.path.normpath(data_dir))
        return ml_path, content_path, data_name
    data_name = args.data
    ml_path = os.path.join(root, f"data/ml_{data_name}.csv")
    content_path = os.path.join(root, f"node_feature/{data_name}.content")
    return ml_path, content_path, data_name
