"""
消融：同 batch 内相同源节点 u 的两条边，对源端 getEmbed 做余弦一致性（1 - cos），与 BCE 加权相加。
...
_stc_args, _argv_rest = _stc_p.parse_known_args()
...
exp_name = f"{data_name}_stc"
early_stopping = EarlyStopping(dn=exp_name, max_round=8)
MODEL_SAVE_PATH = str(paths.model_save_path(exp_name))


def _student_temporal_consistency_loss(batch_data, src_embed):
    """batch 内同 u 的索引两两组合，对源嵌入做 1-cos。"""
    u = batch_data["idx"][:, 0].long().cpu().tolist()
    groups = defaultdict(list)
    for i, uid in enumerate(u):
        groups[int(uid)].append(i)
    if not groups:
        return src_embed.new_tensor(0.0)
    tot = src_embed.new_tensor(0.0)
    npairs = 0
    for g in groups.values():
        ...
        for a in range(len(g)):
            for b in range(a + 1, len(g)):
                ia, ib = g[a], g[b]
                sim = F.cosine_similarity(src_embed[ia : ia + 1], src_embed[ib : ib + 1], dim=-1)
                tot = tot + (1.0 - sim.squeeze(0))
                npairs += 1
    if npairs == 0:
        return src_embed.new_tensor(0.0)
    return tot / npairs
...
def eval_epoch(data, batch_size, model, ctx_sample, tmp_sample, rand_sampler):
    ...
    loss, acc, ap, auc = [], [], [], []
    num_batch_local = math.ceil(num_instance / batch_size)
    with torch.no_grad():
        model.eval()
        for k in range(num_batch_local):
            s_idx = k * batch_size
            ...
            fake_tmp_seq, fake_tmp_mask, fake_tmp_ts = get_unique_node_sequence(
                fake_batch_data, edges, tmp_sample, interaction_list, flag=False
            )
            ...
            pos_prob, neg_prob = st_model.linkPredict(
                context_feature,
                batch_ngh_mask,
                temporal_feature,
                batch_node_seq_mask,
            ...
            pred_score = np.concatenate(
                [(pos_prob).cpu().detach().numpy(), (neg_prob).cpu().detach().numpy()]
            )
...
print(
    "[ablation student_temporal_consistency] lambda_tc=",
    lambda_tc,
    "checkpoint->",
    MODEL_SAVE_PATH,
)
...
for m in range(1):
    print("**************************************************")
    print(
        "run {}------ctx:{},tmp:{},dim:{},N:{},dropout:{}".format(
            m, ctx_sample, tmp_sample, indim, N, dropout
        )
    )
    print("**************************************************")
    ...
    for epoch in tqdm(range(n_epoch)):
        ...
        for k in range(num_batch):
            ...
            train_rand_sampler = RandEdgeSampler(batch_data["idx"][:, 1])
            ...
            src_embed = st_model.getEmbed(
                context_feature, temporal_feature, batch_ngh_mask, batch_node_seq_mask
            )
            loss_tc = _student_temporal_consistency_loss(batch_data, src_embed)
            ...
            total_loss = st_loss + lambda_tc * loss_tc
        ...
        val_acc, val_ap, val_loss, val_auc = eval_epoch(
        ...
        if early_stopping(val_ap, st_model):
            ...
            best_model_path = str(paths.checkpoint_ckpt_path(exp_name))
            st_model.load_state_dict(torch.load(best_model_path, map_location=device))
            ...
            break
    test_acc, test_ap, test_loss, test_auc = eval_epoch(
        test_data, BATCH_SIZE, st_model, ctx_sample, tmp_sample, test_rand_sampler
    )
    nn_test_acc, nn_test_ap, nn_test_loss, nn_test_auc = eval_epoch(
    ...
    log_training_result(
        run_type="ablation_student_temporal",
        data_name=data_name,
        ...
        extra={"model_save": MODEL_SAVE_PATH, "exp_name": exp_name},
    )
