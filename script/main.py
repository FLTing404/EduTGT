...
edges, num_nodes, nodes_list, node_time, train_data, test_data, val_data, nn_test_data, nn_val_data = Dataset(
    file=edges_file, split_by_ui=getattr(args, 'split_by_ui', False), random_state=args.seed)
...
for m in range(1):
    ...
    # 按 code/result/<data_dir>/ 格式保存结果 CSV（与基线一致）
    CODE_ROOT = os.path.dirname(SCRIPT_DIR)
    result_dir = os.path.join(CODE_ROOT, 'result', data_name)
    ...
    results_path = os.path.join(result_dir, f'script_results_{data_name}.csv')
